...
<div class="news-create">
    ...
    <?php $form = ActiveForm::begin(['action' =>['/admin/categories-link/create', 'category_id' => $category->id], 'method' => 'post']); ?>
    <?= $form->field($model, 'owner_category_id')->hiddenInput(['value' => $category->id])->label(''); ?>
...
<?php $this->registerJs('
    ...
    $(document).ready(function(){
        ...
        if(category_root_id){
            refreshSubCategory(local, category_root_id);
        }
    });
    
    function refreshSubCategory(local, category_root_id)
    {
        $.ajax({
            ...
            success : function(data){
                ...
                if(data){
                    $.each(data.categories, function(key, category){
                        htmlOption += "<option value=\""+category.id+"\">"+category[("title_"+local)]+"</option>";
                    });
                }
